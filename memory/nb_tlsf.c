...
*/
static const size_t block_header_overhead = sizeof(block_header_t *);
...
static TLSF_INLINE size_t block_size(const block_header_t *block)
{
	return block->size &
	       ~(block_header_free_bit | block_header_prev_free_bit);
}

static TLSF_INLINE void block_set_size(block_header_t *block, size_t size)
{
...
static TLSF_INLINE int block_is_last(const block_header_t *block)
{
	return block_size(block) == 0;
}

static TLSF_INLINE int block_is_free(const block_header_t *block)
{
	return tlsf_cast(int, block->size &block_header_free_bit);
}

static TLSF_INLINE void block_set_free(block_header_t *block)
{
	block->size |= block_header_free_bit;
}

static TLSF_INLINE void block_set_used(block_header_t *block)
{
	block->size &= ~block_header_free_bit;
}

static TLSF_INLINE int block_is_prev_free(const block_header_t *block)
{
	return tlsf_cast(int, block->size &block_header_prev_free_bit);
}

static TLSF_INLINE void block_set_prev_free(block_header_t *block)
{
	block->size |= block_header_prev_free_bit;
}

static TLSF_INLINE void block_set_prev_used(block_header_t *block)
{
	block->size &= ~block_header_prev_free_bit;
}

static TLSF_INLINE block_header_t *block_from_ptr(const void *ptr)
{
	return tlsf_cast(block_header_t *,
			 tlsf_cast(unsigned char *, ptr) - block_start_offset);
}

static TLSF_INLINE void *block_to_ptr(const block_header_t *block)
{
...
static TLSF_INLINE block_header_t *offset_to_block(const void *ptr, size_t size)
{
	return tlsf_cast(block_header_t *, tlsf_cast(tlsfptr_t, ptr) + size);
}

/* Return location of previous block. */
static TLSF_INLINE block_header_t *block_prev(const block_header_t *block)
{
...
static TLSF_INLINE block_header_t *block_next(const block_header_t *block)
{
...
static TLSF_INLINE block_header_t *block_link_next(block_header_t *block)
{
...
static TLSF_INLINE void block_mark_as_free(block_header_t *block)
{
...
static TLSF_INLINE void block_mark_as_used(block_header_t *block)
{
	block_header_t *next = block_next(block);
...
static TLSF_INLINE size_t align_up(size_t x, size_t align)
{
...
static TLSF_INLINE size_t align_down(size_t x, size_t align)
{
	tlsf_assert(0 == (align & (align - 1)) &&
		    "must align to a power of two");
	return x - (x & (align - 1));
...
static TLSF_INLINE void *align_ptr(const void *ptr, size_t align)
{
...
*/
static TLSF_INLINE size_t adjust_request_size(size_t size, size_t align)
{
	size_t adjust = 0;
	if (size) {
...
static TLSF_INLINE void mapping_insert(size_t size, int *fli, int *sli)
{
	int fl, sl;
...
static TLSF_INLINE void mapping_search(size_t size, int *fli, int *sli)
{
	if (size >= SMALL_BLOCK_SIZE) {
		const size_t round =
...
static TLSF_INLINE void block_remove(control_t *control, block_header_t *block)
{
	int fl, sl;
...
static TLSF_INLINE void block_insert(control_t *control, block_header_t *block)
{
	int fl, sl;
	mapping_insert(block_size(block), &fl, &sl);
...
static TLSF_INLINE block_header_t *
